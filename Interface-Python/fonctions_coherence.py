...
def coherence(fichier_coherent,fichier_test,resolution,seuil_MNT):
    ...
    if data_coherent[0,4]==1:
        message=test_seuil(Matrice_coherent,Matrice_test,seuil_MNT,data_coherent,data_test)
        creation_data_sortie(data_test)
        
    if data_coherent[0,4]==0 or data_coherent[0,4]==2:
        message=test_seuil_premier_motif(Matrice_coherent,Matrice_test,seuil_MNT,data_coherent,data_test)
...
def test_seuil(matrice_coherent,matrice_test,seuil,data_coherent,data_test):
    ...
    for line in range(len(val_noeuds)):
	    ...
	    return("K1=0,K2=0,K3="+str(1-numligne)+",K4="+str(numligne))
	    #return("BadLine_"+numligne+"_"+fichier_test)
...
# si s=1, on refait les deux lignes du noeud invalide             
	...
	# on met 1 pour les lignes valides et 2 pour les lignes invalides
	ind_nonvalide=np.where(data_coherent[:,3]==numligne1) # on recupere num de ligne dans fichier cohérent
	ind_nonvalide=np.where(data_coherent[:,3]==numligne1) # on recupere num de ligne dans fichier cohérent
        data_test[ind_nonvalide,4]=2 # Etat=ligne non valide
        #print('refaire les lignes ',numligne1,' et ',numligne2)
	#return("BADLINE_"+fichier_coherent+"_"+numligne1+"&"+fichier_test+"_"+numligne2)
	return("K1="+str(1-numligne1)+",K2="+str(numligne1)+",K3="+str(1-numligne2)+",K4="+star(numligne2))
...
# si il y a une ligne en commun                 
	    ...
	    #return("badline_"+fichier_coherent+"_"+l1)
	    return("K1="+str(1-l1)+",K2="+str(l1)+",K3=0,K4=0")
        elif l1p==l2p:
            #print('refaire la ligne ',l1p)
	    #return("badline_"+fichier_test+"_"+l1p)
	    return("K1=0,K2=0,K3="+str(1-l1p)+",K4="+str(l1p))
        else:
            #print('refaire toutes les lignes')
	    #return("BADEVERYTHING")
	    return("K1=1,K2=1,K3=1,K4=1")
...
#-----------------------------------------------------------------------------#
# FONCTION qui récupère le nom de la variable en string
def creation_fichier_sortie(array_final,nomprime):
    nom=nomprime[0:-4]
    SOMME=0
